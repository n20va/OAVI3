...
VARIANT = 11
METHOD_NAME = 'Ранговый фильтр (маска "холм")'
WINDOW_SIZE = 3
RANK_NUMERATOR = 12
RANK_DENOMINATOR = 16

# Несколько изображений по требованию задания.
IMAGE_INDICES = [4, 24]
...
def weighted_rank_filter(image: np.ndarray) -> np.ndarray:
    """
    Взвешенный ранговый фильтр с маской "холм":

        1 2 1
        2 4 2
        1 2 1

    Сумма весов = 16, используется ранг 12/16.
    Для каждого пикселя значения окна повторяются в соответствии с весами,
    затем выбирается 12-й элемент отсортированной последовательности.
    """
    kernel = np.array(
        [
            [1, 2, 1],
    ...
    if RANK_NUMERATOR < 1 or RANK_NUMERATOR > RANK_DENOMINATOR:
        raise ValueError("Некорректный ранг фильтра.")
    ...
    # 12/16 -> индекс 11 в 0-based индексации
    rank_index = RANK_NUMERATOR - 1
        ...
        for j in range(width):
            ...
            for wi in range(WINDOW_SIZE):
                for wj in range(WINDOW_SIZE):
                    value = int(window[wi, wj])
                    weight = int(kernel[wi, wj])
                    weighted_values.extend([value] * weight)

            weighted_values.sort()
            result[i, j] = weighted_values[rank_index]
...
def write_report(cases: list[CaseResult]) -> None:
    ...
    lines.append("")
    lines.append(
        f'### Вариант {VARIANT}: {METHOD_NAME}, окно {WINDOW_SIZE}x{WINDOW_SIZE}, ранг {RANK_NUMERATOR}/{RANK_DENOMINATOR}'
    )
    lines.append("")
    ...
    lines.append("```")
    lines.append("")
    lines.append('Ранговый фильтр с весовой маской "холм":')
    lines.append("")
    lines.append("```text")
    lines.append("1 2 1")
    lines.append("2 4 2")
    ...
    lines.append("```")
    lines.append("")
    lines.append("Значения пикселей окна повторяются в соответствии с весами маски,")
    lines.append(f"после сортировки выбирается элемент ранга {RANK_NUMERATOR}/{RANK_DENOMINATOR}.")
    lines.append("")
    lines.append("```text")
    lines.append("F(x, y) = Rank_12(WeightedWindow(x, y))")
    lines.append("```")
    lines.append("")
    lines.append("Разностные изображения:")
    lines.append("")
    ...
    for case in cases:
        ...
        lines.append(
            f'| №{case.case_no} (индекс {case.image_index}) | {size} | {METHOD_NAME}, окно {WINDOW_SIZE}x{WINDOW_SIZE}, ранг {RANK_NUMERATOR}/{RANK_DENOMINATOR} |'
        )
    lines.append("")
    lines.append("### Выводы")
    lines.append("")
    lines.append(f'1. Реализован ранговый фильтр (вариант {VARIANT}) с весовой маской "холм".')
    lines.append(f"2. Использован ранг {RANK_NUMERATOR}/{RANK_DENOMINATOR} для выбора результирующего значения пикселя.")
    lines.append("3. Получены отфильтрованные изображения в полутоне и монохроме.")
    lines.append("4. Сформированы разностные изображения: модуль разности для полутона и XOR для монохрома.")
...
def main() -> None:
    ...
    for case_no, image_index in enumerate(IMAGE_INDICES, start=1):
        ...
        gray_filtered = weighted_rank_filter(gray)
        ...
        mono_filtered = weighted_rank_filter(mono)
    ...
    print(f'Метод: {METHOD_NAME}, окно {WINDOW_SIZE}x{WINDOW_SIZE}, ранг {RANK_NUMERATOR}/{RANK_DENOMINATOR}')
